...
Component({
  ...
  methods: {
    // 删除商品（右滑）：必须用 spuId/skuId 字符串，对象 dataset 在真机不可靠
    deleteGoods(e) {
      const { spuId, skuId } = e.currentTarget.dataset;
      if (!spuId || !skuId) {
        Toast({ context: this, selector: '#t-toast', message: '商品信息缺失' });
        return;
      }
      this.triggerEvent('delete', { goods: { spuId: String(spuId), skuId: String(skuId) } });
    },
    ...
    overlimit(e) {
      const text = e.detail.type === 'minus' ? '该商品数量不能再减少了' : '同一商品最多购买999件';
      Toast({
        context: this,
        selector: '#t-toast',
        message: text,
