...
Page({
  ...
  initData(resData) {
    ...
    this.setData({
      settleDetailData: {
        ...data,
        availableCoupons: resData.availableCoupons || [],
        selectedUserCouponId: resData.selectedUserCouponId || null,
        totalCouponAmount: resData.totalCouponAmount || '0',
        totalPayAmount: resData.totalPayAmount,
        totalSalePrice: resData.totalSalePrice,
        totalPromotionAmount: resData.totalPromotionAmount || '0',
      },
    });
  ...
  onCoupons(e) {
    ...
    const tempSubmitCouponList = (submitCouponList.length
      ? submitCouponList
      : [{ storeId, couponList: [] }]
    ).map((storeCoupon) => {
      const sid = storeCoupon.storeId || storeId;
      return {
        storeId: sid,
        couponList: String(sid) === String(storeId) ? selectedList || [] : storeCoupon.couponList || [],
      };
    });
