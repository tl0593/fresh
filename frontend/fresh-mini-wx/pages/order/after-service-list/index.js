...
Page({
  ...
  getAfterServiceList(statusCode = -1, reset = false) {
    ...
    return getRightsList(params)
        ...
        if (res && res.data && res.data.dataList) {
          dataList = (res.data.dataList || []).map((_data) => {
            return {
              ...
              typeDescIcon:
                (_data.rights && _data.rights.rightsType) === ServiceType.ONLY_REFUND
                  ? 'money-circle'
                  : 'return-goods-1',
              status: _data.rights.rightsStatus,
              statusName: _data.rights.userRightsStatusName || _data.rights.rightsStatusName || '',
              ...
              goodsList: (_data.rightsItem || []).map((item, i) => ({
                ...
                rightsQuantity: item.rightsQuantity || 1,
              })),
              storeId: _data.storeId,
              buttons: _data.buttonVOs || [],
              logisticsNo: (_data.logisticsVO && _data.logisticsVO.logisticsNo) || '',
              logisticsCompanyName: (_data.logisticsVO && _data.logisticsVO.logisticsCompanyName) || '',
              logisticsCompanyCode: (_data.logisticsVO && _data.logisticsVO.logisticsCompanyCode) || '',
              remark: (_data.logisticsVO && _data.logisticsVO.remark) || '',
      ...
      .catch((err) => {
        console.warn('[Fresh] 售后列表加载失败', err && err.message);
        this.setData({
          listLoading: 3,
