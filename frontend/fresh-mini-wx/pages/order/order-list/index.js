...
Page({
  ...
  data: {
    tabs: [
      ...
      { key: OrderStatus.PENDING_DELIVERY, text: '待配送', info: '' },
      { key: OrderStatus.WAIT_PICKUP, text: '待自提', info: '' },
      { key: OrderStatus.COMPLETE, text: '已完成', info: '' },
      { key: OrderStatus.CANCELED, text: '已取消', info: '' },
    ],
    curTab: -1,
    orderList: [],
    listLoading: 0,
          ...
          orderList = (res.data.orders || []).map((order) => {
            return {
              ...
              goodsList: (order.orderItemVOs || []).map((goods) => {
                const rawImg = goods.goodsPictureUrl || '';
                const isLocal =
                  !rawImg ||
                  /\/api\/goods\/upload\//i.test(rawImg) ||
                  rawImg.indexOf('127.0.0.1') >= 0 ||
                  rawImg.indexOf('localhost') >= 0;
                return {
                  id: goods.id,
                  thumb: isLocal ? rawImg : cosThumb(rawImg, 70),
                  title: goods.goodsName,
                  skuId: goods.skuId,
                  spuId: goods.spuId,
                  isCommented: goods.isCommented,
