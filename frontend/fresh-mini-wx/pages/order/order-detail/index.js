...
Page({
  ...
  /** 跳转按商品评价（多商品时在评价页切换） */
  navToCommentCreate() {
    const list = (this.data._order && this.data._order.goodsList) || [];
    const pending = list.find((g) => !(g.isCommented === 1 || g.isCommented === true || g.isCommented === '1'));
    const orderItemId = (pending && pending.id) || (list[0] && list[0].id) || '';
