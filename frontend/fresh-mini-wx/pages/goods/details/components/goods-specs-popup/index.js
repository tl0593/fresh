...
Component({
  ...
  methods: {
    initData() {
      ...
      const selectedSku = {};
      // 仅一个可选规格值时自动选中，避免改数量后仍提示「请选择规格」
      specList.forEach((item) => {
        const options = (item.specValueList || []).filter(
          (v) => !v.hasStockObj || v.hasStockObj.hasStock !== false,
        );
        if (options.length === 1) {
          selectedSku[item.specId] = options[0].specValueId;
          item.specValueList.forEach((v) => {
            v.isSelected = String(v.specValueId) === String(options[0].specValueId);
          });
        } else {
    ...
    resolveSkuStock(item) {
      if (!item) return 0;
      if (item.quantity != null) return Number(item.quantity) || 0;
      if (item.stockInfo && item.stockInfo.stockQuantity != null) {
        return Number(item.stockInfo.stockQuantity) || 0;
      }
    ...
    checkSkuStockQuantity(specValueId, skuList) {
      ...
      (skuList || []).forEach((item) => {
        const stock = this.resolveSkuStock(item);
        (item.specInfo || []).forEach((subItem) => {
          if (String(subItem.specValueId) === String(specValueId) && stock > 0) {
            const subArray = [];
      ...
      // 无库存字段时放行（避免误判售罄导致无法加购）
      if (!(skuList || []).length) {
        hasStock = true;
      } else if (!(skuList || []).some((s) => this.resolveSkuStock(s) > 0)
          && (skuList || []).every((s) => s.quantity == null && !(s.stockInfo && s.stockInfo.stockQuantity != null) && s.stock == null)) {
        hasStock = true;
        (skuList || []).forEach((item) => {
          (item.specInfo || []).forEach((subItem) => {
            if (String(subItem.specValueId) === String(specValueId)) {
              array.push((item.specInfo || []).map((specItem) => specItem.specValueId));
            }
          });
        });
      }
    ...
    handleBuyNumChange(e) {
      const value = Number((e.detail && e.detail.value) != null ? e.detail.value : e.detail);
      const buyNum = Number.isFinite(value) && value > 0 ? value : 1;
